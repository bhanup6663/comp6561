...
class Graph:
    ...
    def to_csv(self, n, r, upper_cap):
        filename = "generated_graph_n%s_r%s_cap%s.csv" % (n, r, upper_cap)
        ...
        for i in range(len(augmenting_path) - 1):
            ...
            for j, (neighbor, edge_capacity) in enumerate(graph.edges[u]):
                if neighbor == v and edge_capacity > 0:
                    ...
                    break
            if not edge_found:
                print("Error: No valid edge found for path %s" % augmenting_path)
                break
...
def dfs_like(graph, source, sink):
    ...
    if dfs(source):
        result_path = [source] + [edge[1] for edge in path]
        return result_path
    else:
        return None
...
def run_simulations(n, r, upper_cap):
    ...
    for algorithm in augmenting_algorithms:
        print("------------")
        print("\nAlgorithm: %s" % algorithm.__name__)
        saved_graph = Graph.from_csv(n, r, upper_cap)
        total_edges = sum(len(edges) for edges in saved_graph.edges.values())
        start_time = time.time()
        paths, total_length, total_proportional_length = FordFulkerson(saved_graph, saved_graph.source,
                                                                       saved_graph.sink, algorithm)
        ...
        print("Total Edges in the Graph: %s" % total_edges)
        print("Paths: %s" % paths)
        print("Mean Length: %s" % mean_length)
        print("Mean Proportional Length: %s" % mean_proportional_length)
...
if __name__ == "__main__":
    ...
    for n, r, upper_cap in simulations_params:
        ...
        print("Simulation for n=%s, r=%s, upperCap=%s" % (n, r, upper_cap))
